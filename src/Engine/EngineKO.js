...
export function engine(fen , isWhite = true){
    ...
    let maxdepth = 4;
    const ExpectedEvals = (Math.pow(moves.length , 5)) / 50
    
    // console.log("expected : " , ExpectedEvals)

    // maxdepth = (ExpectedEvals > 20000) ? 3 : 4;

    // console.log("intinalized : " , moves.length , "depth : " , maxdepth)
    for(const move of moves){
        chess.play(move)
        if(false){
            MainChess.undo()
...
// engine("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1" , false) // 18 move


// console.timeEnd("time")
// console.time("time")

// engine("rnbqkbnr/pppppppp/8/8/7P/8/PPPPPPP1/RNBQKBNR b KQkq - 0 1" , true) // 18 move
