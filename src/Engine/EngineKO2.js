...
function evaluate(fen , isWhite , depth){
    ...
    if(["c1" , "g1" , "c8" , "g8"].includes(ourKing)){
        score += 0.75
    }
    if(numberOfPieces <= 5){
...
function quietSearch(fen, alpha, beta, isMaximizing , maxDepth ,  depth , isWhite , mainDepth) {
    if(depth >= maxDepth){
        return evaluate(fen, isWhite , depth + mainDepth);
    }
    let stand_pat = evaluate(fen, isWhite , depth + mainDepth);

    const base = new Position(fen)
    if (isMaximizing) {
        if (stand_pat >= beta) {
    ...
    const captures = base.moves().filter(move => {
        return move.isCapture()
    });
    for (const move of captures) {
        let chess = new Position(fen)
        chess.play(move);
        let score = quietSearch(chess.fen(), alpha, beta, !isMaximizing , depth + 1 , isWhite , mainDepth);
...
function minimax(fen , depth , isMaximizing, steps , alpha , beta , skips , isWhite , isOVERSEARCH , maxDepth , FENcounter){
    steps.count++;
    ...
    if(base.isCheckmate() && !isMaximizing){
        ...
        return depth - 1000
    }else if(base.isDead() || base.isStalemate() || FENcounter.get(fen) == 2){
        return 0;
    }else if(depth == maxDepth){
        if(isOVERSEARCH){
            return quietSearch(fen , alpha , beta , isMaximizing , isOVERSEARCH , 0 , isWhite , depth)
        }
    ...
    if(TT.has(fen) && TT.get(fen)[1] == maxDepth){
        ...
        if(isMaximizing){
            ...
            for(const move of moves){
                let chess = new Position(fen)
                chess.play(move);
                let score = minimax(chess.fen() , depth + 1 , false , steps , alpha , beta , skips , isWhite , isOVERSEARCH , maxDepth , FENcounter);
            ...
            for(const move of moves){
                let chess = new Position(fen)
                chess.play(move)
                let score = minimax(chess.fen() , depth + 1 , true , steps , alpha , beta , skips , isWhite , isOVERSEARCH , maxDepth , FENcounter);
...
export function engine(fen, isWhite = true, timeLimit = 10000 , FENcounter = new Map()) {
    let chess = new Position(fen);
    let bestMove = null;
    let bestScore = -Infinity;
    let maxDepth = 0;
    ...
    while (Date.now() - startTime < timeLimit) {
        ...
        // Conservative check: If the remaining time is less than 3x the previous depth's time, stop
        if (currentTime + 3 * previousDepthTime > timeLimit) {
            // console.log("Not enough time for the next depth. Stopping at depth:", maxDepth);
            break;
        }
        ...
        for (const move of moves) {
            ...
            chess = new Position(fen); // Reset the position
        }

        // Measure the time spent on the current depth and update the estimate
        previousDepthTime = Date.now() - depthStartTime;
    }

    // console.log("Max depth reached:", maxDepth);
    // console.log(`${bestMove.from()}${bestMove.to()}`, bestScore, "evals:", steps.count);
...
// engine("r2q1rk1/ppp2ppp/2n2n2/2bpp3/2BPP3/2N2N2/PPP2PPP/R1BQ1RK1 w - - 0 1" , false , 10000) // 39 move
...
// console.log(engine("rnbqkbnr/pppppppp/8/8/8/5N2/PPPPPPPP/RNBQKB1R w KQkq - 0 1" , false , 10000)) // 18 move
