...
function minimax(fen , depth , isMaximizing, steps , alpha , beta , skips , isWhite , isOVERSEARCH , maxDepth , FENcounter , sorting = false){
    steps.count++;
    ...
    if(TT.has(fen) && TT.get(fen)[1] == maxDepth){
        ...
        if(isMaximizing){
            ...
            for(const move of moves){
                let chess = new Position(fen)
                chess.play(move);
                let score = minimax(chess.fen() , depth + 1 , false , steps , alpha , beta , skips , isWhite , isOVERSEARCH , maxDepth , FENcounter , sorting);
            ...
            for(const move of moves){
                let chess = new Position(fen)
                chess.play(move)
                let score = minimax(chess.fen() , depth + 1 , true , steps , alpha , beta , skips , isWhite , isOVERSEARCH , maxDepth , FENcounter , sorting);
